...
public class NatureApp {
	...
	//Set.add?
	natureDiary.getCarnivores.forEach(System.out::println);
	
	//1) Nombre de plantes et d'animaux rencontrés
	natureDiary.getAnimalCount.for;
	
	natureDiary.getPlantCount().for;
	
	for Set<String> PlantDiet &&set.iterator();
			getAn
	System.out.println("You have met:" + ""+getAnimalCount() animals && getPlantCount() plants");
	
	//2) Liste pas triée des animaux et plantes(Set), methode printNotebook van Notebook instantie
			
	//3) Print lijsten séparées de Carnivores, omnivores et herbivores.//forEach objet.getcarnivores.forEach
			for 
	...
	//4)Trier les animaux et les plantes à partir des méthodes sortAnimalsByName et SortPlantsByName de l'instance Notebook		
			
			System.out.println("Sort Animals by name": sortAnimalsByName)
			
			
			//Liste triée des animaux et plantes (List)
					Sort Plants by name
